...
md.push(`>（${communityLabel}，由「带百度 AK 真实体检」导出并内置为仓库示例数据）。`);
...
md.push('');
md.push('# 2) 浏览器打开');
md.push('#    带 AK 在线体检：                配置 config.local.js 后打开首页，输入地址→开始体检');
md.push('#    线上演示链接：                  由 CI 注入 AK 部署后的 GitHub Pages 地址');
md.push('');
md.push('# 3) 示例数据（仓库内置真实体检快照）');
