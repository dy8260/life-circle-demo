...
md.push(`>（${communityLabel}，由系统在「带百度 AK 真实体检」后点「📤 导出演示数据」生成）。`);
...
md.push('');
md.push('# 3) 导出真实数据覆盖示例');
md.push('#    真实体检完成后，点右上角「📤 导出演示数据」→ 下载 sample-community.json');
md.push('#    → 覆盖 data/sample-community.json → 重新打开即加载真实数据');
